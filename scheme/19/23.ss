   ...
   (lambda (done)
     ...
     (define (send-packets)
       (let loop ((Q Q))
         (match Q
           ((255 _ y z ...) (done y))
           ((a x y z ...) (send-input* (vector-ref net a) (list x y)) (loop z))
           (() (void))))
       (set! Q '()))
     (define (feed-1)
       (vector-for-each (lambda (nic) (send-input nic -1)) net))
     (let loop ()
       (advance)
       (collect-packets)
       (if (null? Q)
  ...
  (call/cc
   (lambda (done)
     ...
     (define (send-packets)
       (let loop ((Q Q))
         (match Q
           ((255 x y z ...) (set! nat (list x y)) (loop z))
           ((a x y z ...) (send-input* (vector-ref net a) (list x y)) (loop z))
           (() (void))))
       (set! Q '()))
     (define (check-idle)
