...
(advent-year 24) (advent-day 24)

(define input
  (parse-advent lines-raw))

(define (parse-statement s)
  (define (string->op op)
    (string->symbol
     (string-append "log" (string-downcase op))))
  (match (string-split s ": ")
    ((x b)
     `(,(string->symbol x) ,(string->number b)))
    ((x op y "->" z)
     `(,(string->symbol z)
       (,(string->op op) ,(string->symbol x) ,(string->symbol y))))
    ('() #f)
    (_ #f)))

(define statements
  (filter-map parse-statement input))
