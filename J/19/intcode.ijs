...
step =: 3 : 0
 ...
 case. 1 do. 'ok' [ ip=: ip+4 [ program=: (+/ ref 1 2) (addr 3)} program
 case. 2 do. 'ok' [ ip=: ip+4 [ program=: (*/ ref 1 2) (addr 3)} program
 case. 3 do. if. #in do. program=: ({.in) (addr 1)} program
                         'ok' [ ip =: ip + 2 [ in =: }. in
	     else. 'blocked' end.
 case. 4 do. 'out' [ ip=: ip+2 [ out=: out,ref 1
 case. 5 do. if. ref 1 do. ip=: ref 2 else. ip=: ip+3 end. 'ok'
 case. 6 do. if. 0 = ref 1 do. ip=: ref 2 else. ip=: ip+3 end. 'ok'
 case. 7 do. 'ok' [ ip=: ip+4 [ program=: ((ref 1) < (ref 2)) (addr 3)} program
 case. 8 do. 'ok' [ ip=: ip+4 [ program=: ((ref 1) = (ref 2)) (addr 3)} program
 case. 9 do. 'ok' [ ip=: 2+ip [ rp=: rp + ref 1
 case. 99 do. 'halted' end.
)
