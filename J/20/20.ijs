...
M =: {{ , h #~ ({.y) ~: {."1 h =. 4 $. $. H -:"_2 _ {. X{~<y }} NB. connect squares
R =: {{ (#~ ((=&# ~.) {."1)"2) M^:(i.-N)"1 y,.i.8}} NB. row
T =: {{ r #~ -. +./"1 {."_2 (x-.y) e.~ r =. R y }}  NB. column given row x
ST =: {{ if.({:"1 x)-:{."1 y do. x,.y NB. stitch together strips (columns)
         elseif. ({."1 x)-:{."1 y do. (|."1 x),.y
         elseif. ({."1 x)-:{:"1 y do. (|."1 x) ,. |."1 y
         else. x,.|."1 y end.}}
SEA =: {{IMG=.ST/,/"_1 X{~<"1{."_1(F2&T)"0 F2=.{."1{.R y NB. assemble image
         '#'=,./^:2(2 2$10)(}.@:}:"_1@:}.@:}:);._3 IMG}}
...
MON =: (1 1,:$SM)&(SMC=+/@:,@:(SM&*);._3) NB. find monsters in given image
