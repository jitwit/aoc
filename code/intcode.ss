...
(define (pretty-opcode M)
  (let* ((ip (M 'ip)) (op (M 'ref ip)))
    (case (mod op 100)
      ((1) `(,ip (add (,(pretty-mode op 1) ,(M 'ref (+ ip 1)))
                      (,(pretty-mode op 2) ,(M 'ref (+ ip 2)))
                      (,(pretty-mode op 3) ,(M 'ref (+ ip 3))))))
      ((2) `(,ip (mul (,(pretty-mode op 1) ,(M 'ref (+ ip 1)))
                      (,(pretty-mode op 2) ,(M 'ref (+ ip 2)))
                      (,(pretty-mode op 3) ,(M 'ref (+ ip 3))))))
      ((3) `(,ip (read (,(pretty-mode op 1) ,(M 'ref (+ ip 1))))))
      ((4) `(,ip (out (,(pretty-mode op 1) ,(M 'ref (+ ip 1))))))
      ((5) `(,ip (jnz (,(pretty-mode op 1) ,(M 'ref (+ ip 1)))
                      (,(pretty-mode op 2) ,(M 'ref (+ ip 2))))))
      ((6) `(,ip (jez (,(pretty-mode op 1) ,(M 'ref (+ ip 1)))
                      (,(pretty-mode op 2) ,(M 'ref (+ ip 2))))))
      ((7) `(,ip (lt (,(pretty-mode op 1) ,(M 'ref (+ ip 1)))
                     (,(pretty-mode op 2) ,(M 'ref (+ ip 2)))
                     (,(pretty-mode op 3) ,(M 'ref (+ ip 3))))))
      ((8) `(,ip (eq (,(pretty-mode op 1) ,(M 'ref (+ ip 1)))
                     (,(pretty-mode op 2) ,(M 'ref (+ ip 2)))
                     (,(pretty-mode op 3) ,(M 'ref (+ ip 3))))))
      ((9) `(,ip (rb (,(pretty-mode op 1) ,(M 'ref (+ ip 1))))))
      ((99) `(,ip (return))))))
